...
export class StoreRoom {
  ...
  private tick(): void {
    ...
    const customersPayload: Record<string, { position: Vector3D; rotationY: number; state: string; heldProductId: string | null }> = {};
    ...
    const monstersPayload: Record<string, { position: Vector3D; rotationY: number; state: string; health: number; maxHealth: number; type: string }> = {};
  ...
  private updateCustomers(dt: number): void {
    if (this.shiftPhase === 'DAY') {
      const spawnInterval = this.teamUnlocks.has('bakery_dept')
        ? SHIFT_CONFIG.CUSTOMER_SPAWN_INTERVAL_BAKERY
        : SHIFT_CONFIG.CUSTOMER_SPAWN_INTERVAL_BASE;

      this.customerSpawnTimer += dt;
      if (this.customerSpawnTimer >= spawnInterval && this.customers.size < SHIFT_CONFIG.MAX_CUSTOMERS) {
  ...
  private defeatMonster(monsterId: string, killerPlayerId?: string, customKillerName?: string): void {
    ...
    const bonus = monster.type === 'BRUTE' ? 30 : 15;
    if (killerPlayerId) {
  ...
  private updateFlyingBoxes(dt: number): void {
    ...
    for (const [boxId, flight] of Array.from(this.flyingBoxes.entries())) {
      ...
      if (speed > 1.2) {
        ...
        // 2. Check Collision with Players (3s Ragdoll Stun)
